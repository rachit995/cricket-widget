    ...
    <div class="score-card-box">
      <div id="score-card-box-front" class="score-card-box-faces">
        <div class="score-card-information">
          ...
          166/6 (20/20)
          <img src="images/cricket-icon.svg" class="cricket-icon" alt="cricket icon" />
        </div>
      </div>
      <div id="score-card-box-right" class="score-card-box-faces">
        <img src="images/1.png" class="player-image" alt="player photo" />
      </div>
...
class ScoreCard extends HTMLElement {
  constructor () {
    ...
    dragElement(this.shadowRoot.querySelector('#score-card-wrapper'));
  }

  toggleVisibility() {
    this.visibility = !this.visibility;
    const box = this.shadowRoot.querySelector('.score-card-box')
    const toggleBtn = this.shadowRoot.querySelector('#visibility-button')
    if (this.visibility) {
  ...
  toggleZoom() {
    this.zoom = !this.zoom;
    const info = this.shadowRoot.querySelector('#score-card-wrapper')
    const toggleBtn = this.shadowRoot.querySelector('#zoom-button')
    if (this.zoom) {
  ...
  connectedCallback() {
    this.shadowRoot.querySelector('#visibility-button').addEventListener('click', () => this.toggleVisibility())
    this.shadowRoot.querySelector('#zoom-button').addEventListener('click', () => this.toggleZoom())
  }

  disconnectedCallback() {
    this.shadowRoot.querySelector('#visibility-button').removeEventListener()
    this.shadowRoot.querySelector('#zoom-button').removeEventListener()
  }
}
